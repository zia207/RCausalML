# DAG-GNN: DAG Structure Learning with Graph Neural Networks {.unnumbered}

DAG-GNN is a method for learning the structure of a Directed Acyclic Graph (DAG) from observational data, using a deep generative model that incorporates graph neural networks (GNNs). It extends the NOTEARS framework to handle nonlinear relationships and complex variable types, making it a powerful tool for causal discovery and inference in real-world datasets. This notebook demonstrates how to implement and apply DAG-GNN for both synthetic and real-world data, including validation of the learned structure and estimation of causal effects. The implementation is based on the original paper by Yu et al. (ICML 2019) and includes detailed explanations of the model architecture, training procedure, and evaluation metrics.

## Implementation in R

The `CausalML` R package provides tools for causal inference, structure learning, and treatment effect estimation. In particular, the package includes an R port of DAG-GNN (`R/dag_gnn.r`), which leverages the `torch` library to enable deep neural network-based learning of Directed Acyclic Graphs (DAGs) from data. `CausalML` offers a unified interface to instantiate, train, and evaluate models for causal structure discovery and effect estimation.
...
The model in `R/dag_gnn.r` (v2) uses a VAE with learnable adjacency **A**, MLP encoder/decoder, and **tanh(A)×0.5** as the effective adjacency (replacing the earlier sinh-based formulation for numerical stability). Acyclicity is enforced via **matrix_poly** so that $h(\mathbf{A})=0$ iff the graph is a DAG. Optional structural penalties (A_connect, A_positive) are available. See the Sachs and Wine case studies for full training loops with **throttled c updates** and **gradient clipping**.
...
### Training with augmented Lagrangian (paper Eqs. 14–16)

Minimize $-\mathrm{ELBO}$ subject to $h(\mathbf{A}) = 0$. We use the **stable training** from `R/dag_gnn.r` (FIX 2a–2c): **throttled c** (updated every 25 epochs, cap 1e6) so the penalty does not explode; **gradient clipping** (max_norm 1.0); **tryCatch** on forward to skip singular steps; **skip NaN** loss steps so $\lambda$ is not poisoned. Define $L_c = f(A,\theta) + \lambda h(A) + \frac{c}{2} h(A)^2 + \text{Huber}(A)$. Update $\lambda$ every step; update $c$ only when $|h(A_k)| > \gamma |h(A_{k-1})|$ at the throttle interval.
...
DAG-GNN turns the notoriously hard DAG-learning problem into a differentiable VAE + GNN optimization. The R implementation in `R/dag_gnn.r` (v2) uses **tanh(A)×0.5** effective adjacency and **stable training** (throttled penalty *c*, gradient clipping, NaN/singularity guards), so it scales to real data (see Sachs and Wine case studies). In production:
...
-   **R Port**: `R/dag_gnn.r` (v2: tanh×0.5 effective adj, stable training)
...
*Note: This R Quarto notebook was converted from the original Python IPython notebook, maintaining the same structure and functionality while leveraging R's `{torch}`, `{HDclassif}`, and `{tidyverse}` ecosystems.*

## Scientific terminology for beginners

| Term | Simple explanation | Beginner example |
|---|---|---|
| DAG-GNN | Graph neural network approach to learning DAG structures. | Encoder-decoder framework learns adjacency while reconstructing data. |
| Graph neural network | Neural model operating on graph-structured relationships. | Node embeddings update using information from neighboring nodes. |
| Encoder-decoder | Pair of networks for compression and reconstruction. | Encoder maps data to latent codes, decoder reconstructs observations. |
| Effective adjacency | Learned edge-weight matrix after transformations/constraints. | Use transformed weights to infer final directed edges. |
| Nonlinear mechanism | Causal relation that is not well represented by a straight line. | Outcome rises sharply only after treatment exceeds a threshold. |
| Post-processing threshold | Final cutoff applied to learned weights to produce sparse DAG. | Keep edges where score exceeds 0.10. |

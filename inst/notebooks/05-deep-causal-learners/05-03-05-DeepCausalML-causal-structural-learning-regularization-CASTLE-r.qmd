...
"),
  ...
  ggdag(g_dag, use_labels = "name", layout = "stress") +
    geom_dag_nodes(aes(fill = name %in% highlight_vars), color = "black", shape = 21, size = 10) +
...
estimate_ate_adjustment <- function(W, names, trx, trt, try_, tex, tpy, model_name = "", edge_thr = 1e-8) {
  ...
  fit1 <- lm(try_ ~ ., data = as.data.frame(trx[idx_t, par, drop = FALSE]))
  fit0 <- lm(try_ ~ ., data = as.data.frame(trx[idx_c, par, drop = FALSE]))

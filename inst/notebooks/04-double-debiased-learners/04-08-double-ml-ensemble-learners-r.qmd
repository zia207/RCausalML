# 9. DoubleML with Ensemble Learners  {.unnumbered}

This tutorial shows how to use **ensemble learners** and **mlr3pipelines** with DoubleML in R. **RCausalML**’s **`R/DMLearner.R`** provides thin wrappers around the DoubleML data backend—**`doubleml_data_from_matrix()`**, **`doubleml_data_from_data_frame()`**, and **`doubleml_fetch_bonus()`** (delegating to **DoubleML**)—so data construction matches the rest of the RCausalML DoubleML workflow. We combine multiple machine learning algorithms (e.g., lasso, random forest, regression trees), build pipelines for feature engineering and tuning, and pass them as nuisance learners to DoubleML. We follow the [DoubleML R: Ensemble Learners and More with mlr3pipelines](https://docs.doubleml.org/stable/examples/R_double_ml_pipeline.html) example, using **simulated data** and the **Bonus data** set.

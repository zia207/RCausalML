...
*Example:* $X_3 = f(X_1, X_2) + \varepsilon_3$, where $\varepsilon_3$ is random noise independent of $X_1$ and $X_2$.
...
*Example:* $X \to T \to Y$ means X influences treatment T, which influences outcome Y.
...
*Example:* $\tau(x) = \mu_A(x) - \mu_{\text{control}}(x)$ is the contrast between Drug A and the control for a patient with features $x$.
...
Setting a variable to a specific fixed value regardless of what its normal causes say — the $\text{do}(\cdot)$ operator in causal calculus.

*Example:* $\text{do}(T=1)$ forces everyone to receive treatment, even those who would normally not qualify.
...
*Example:* For patient A, $\text{ITE} = Y(1) - Y(0) = 8 - 5 = +3$ days of faster recovery.
...
*Example:* $q(z \mid x, t, y)$ is the estimated distribution over likely latent confounders for one specific patient record.
...
The probability of receiving treatment given a unit's observed covariates: $P(T=1 \mid X)$.
...
*Example:* In double ML, compute $\tilde{Y} = Y - \mathbb{E}[Y \mid X]$ and $\tilde{T} = T - \mathbb{E}[T \mid X]$, then regress $\tilde{Y}$ on $\tilde{T}$ to estimate the causal effect.
...
An intervention that *changes* the parameters or mechanism of a structural equation rather than forcing a fixed value — a weaker form of intervention than $\text{do}(\cdot)$.
...
*Example:* $Y = f(T, X, \varepsilon_Y)$ where $\varepsilon_Y$ is random noise — changing $T$ while holding $\varepsilon_Y$ fixed gives the causal effect of $T$ on $Y$.

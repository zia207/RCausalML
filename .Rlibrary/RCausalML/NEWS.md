...
- **DAG-GNN exports** (`R/causalDeepNet.R`): `make_daggnn`, `daggnn_adj`, `DAGGNN`,
  `get_daggnn_device`, `preprocess_adj`, and `matrix_poly` are exported from
  **RCausalML** (bundled in `R/causalDeepNet.R` alongside NOTEARS).

- **NOTEARS exports** (`R/causalDeepNet.R`): NOTEARS structure-learning helpers
  (`notears_linear`, `notears_nonlinear`, `NotearsMLP`, `NotearsSobolev`,
  `simulate_dag`, `count_accuracy`, and related utilities) are now part of the
  installed **RCausalML** namespace (formerly standalone `R/notears.R`).

- **`uplift_randomForest()`** (`R/uplift_randomForest.R`): unified uplift random
  forest API aligned with Python `UpliftRandomForestClassifier` for heterogeneous
  treatment effects. Supports **classification** (bagged KL / ED / Chi / CTS /
  DDP / IDDP trees) and **regression** (T-learner `ranger` forests or bagged
  IT / CIT trees). Multi-arm experiments fit one forest per non-control arm.
  Wrappers: `uplift_randomForestClassifier()`, `uplift_randomForestRegressor()`.
  Test script: `tests/test-uplift_randomForest.R`.

- **`uplift_xgboost()`** (`R/uplift_xgboost.R`): T-learner XGBoost uplift model
  with the same API shape as `uplift_randomForest()`. Fits separate XGBoost
  outcome models per treatment arm and estimates CATE as the predicted outcome
  difference. Supports binary and multi-arm treatment, classification and
  regression outcomes, and `full_output` predictions. Wrappers:
  `uplift_xgboostClassifier()`, `uplift_xgboostRegressor()`. Requires the
  optional **xgboost** package. Test script: `tests/test-uplift_xgboost.R`.

### Bug fixes

- **`predict.DMLearner` — NA coefficient propagation** (`R/DMLearner.R`): When
  `stats::lm()` drops aliased/collinear columns in the final OLS stage
  (`LinearDML`, `KernelDML`), it sets those coefficients to `NA`. The manual
  `coef %*% phi(X)` multiply now replaces `NA` coefficients with `0` (treating
  dropped directions as zero contribution) before the matrix multiplication,
  preventing all-`NA` CATE predictions. Applied to both the `"lm"` and
  `"kernel"` branches of `predict.DMLearner`.

- **`predict.DMLearner` — finite-value guarantee** (`R/DMLearner.R`): A final
  safety guard at the end of `predict.DMLearner` replaces any residual
  `NA` / `NaN` / `Inf` in the returned prediction vector with the mean of the
  finite values (or `0` if everything is non-finite). This ensures downstream
  callers such as `kernelshap::permshap` and `shapviz` never receive
  non-finite values, fixing the **"No missing SHAP values allowed"** error when
  computing `explain_cate()` on DML models.

- **`rcausalml_predict_numeric` — SHAP pred_fun guard** (`R/shapviz_integration.R`):
  The prediction wrapper used by `explain_cate()` now replaces any `NA` /
  `NaN` / `Inf` in its output with the mean of finite values before returning,
  providing a belt-and-suspenders guarantee that `permshap` / `kernelshap`
  never receives non-finite predictions regardless of the underlying model type.

- **`lst-nmes-dml-fit-compare` Spearman guard** (`tutorials/02-03-meta-learners-continuous-treatment-r.qmd`):
  Added a `complete.cases(M) >= 3` check before the Spearman correlation loop
  so a degenerate-model edge case emits a warning and leaves the matrix as
  `NA` rather than crashing with **"no complete element pairs"**.
...
- **DESCRIPTION**: version **0.3.0** (2026-05-21).

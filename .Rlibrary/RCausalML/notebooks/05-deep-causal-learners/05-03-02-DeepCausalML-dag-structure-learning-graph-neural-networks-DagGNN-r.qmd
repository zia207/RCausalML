...
Minimize $-\mathrm{ELBO}$ subject to $h(\mathbf{A}) = 0$. We use the **stable training** from `R/causalDeepNet.R` (FIX 2a–2c): **throttled c** (updated every 25 epochs, cap 1e6) so the penalty does not explode; **gradient clipping** (max_norm 1.0); **tryCatch** on forward to skip singular steps; **skip NaN** loss steps so $\lambda$ is not poisoned. Define $L_c = f(A,\theta) + \lambda h(A) + \frac{c}{2} h(A)^2 + \text{Huber}(A)$. Update $\lambda$ every step; update $c$ only when $|h(A_k)| > \gamma |h(A_{k-1})|$ at the throttle interval.

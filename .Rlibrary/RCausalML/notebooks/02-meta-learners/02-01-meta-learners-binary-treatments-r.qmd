# 2. Meta-Learner with Binary Treatments {.unnumbered}

Meta-learners are flexible frameworks that allow us to estimate CATE using any regression or classification algorithm as a base learner. For binary treatments, the key idea is to leverage the treatment indicator in different ways to isolate the treatment effect heterogeneity. Binary treatment meta-learners are particularly useful in observational studies where treatment assignment is not random, and we need to adjust for confounding. Each meta-learner has its own strengths and weaknesses, and their performance can vary based on the data characteristics, such as the degree of imbalance in treatment groups and the complexity of the underlying CATE function. 

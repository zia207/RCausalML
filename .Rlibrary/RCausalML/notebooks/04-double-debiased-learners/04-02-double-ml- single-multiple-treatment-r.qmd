# 3.  CausalML DMLearners - Single/multiple Treatment, Synthetic and Observational data
...
|---|---|---|
| CATE (Conditional Average Treatment Effect) | Treatment effect that changes by person or subgroup. | "The treatment helps younger patients more than older patients." |
| Heterogeneous treatment effect | Different units get different treatment effects. | "A discount helps new customers more than loyal customers." |
| Residualization | Removing the part explained by covariates before estimating effect. | "First predict outcome from X, then use leftover part for causal step." |
| Nuisance model | ML model for treatment/outcome prediction used as an intermediate step. | "Random forest predicts treatment assignment probability." |
| Sparse model | A model where only a small subset of features has non-zero influence. | "Out of 100 variables, only 5 drive heterogeneity." |
| ATE (Average Treatment Effect) | Mean treatment effect over the sample. | "Average increase in spending after policy = $30." |
